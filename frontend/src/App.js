...
const PORTRAIT = "https://customer-assets-lqy194kg.emergentagent.net/job_dynamic-layers-1/artifacts/xle0vma9_ChatGPT%20Image%20Aug%209%2C%202026%2C%2002_21_41%20AM.png";
...
function App() {
  ...
  return <div className="portfolio-shell">
    ...
    <main id="top">
      ...
      <section id="work"><div className="wrap"><div className="section-head"><div><div className="eyebrow reveal">02 — Selected Work</div><h2 className="reveal reveal-1">Things I’ve designed<br />and shipped.</h2></div><p className="section-note reveal reveal-2">Four projects spanning AI products, SaaS, mobile, and interface experiments. Click once to preview, twice to open the case study.</p></div><div className="project-list">{projects.map((project, i) => <article className={`project-row reveal reveal-${Math.min(i + 1, 4)} ${expandedProject === i ? "active" : ""}`} key={project.title} onClick={() => { if (expandedProject === i) setActiveProject(i); else setExpandedProject(i); }} data-testid={`project-row-${i}`} onMouseEnter={() => setCursorBig(true)} onMouseLeave={() => setCursorBig(false)}><div className="project-inner"><span className="project-num">0{i + 1}</span><div className="project-titles"><span className="project-cat">{project.category}</span><h3>{project.title}</h3><p className="project-desc">{project.description}</p><div className="project-tags">{project.tags.map((tag) => <span className="tag" key={tag}>{tag}</span>)}</div><span className="project-year">{project.year}</span></div><div className="project-visual"><AbstractVisual index={i} /></div><span className="project-arrow"><ArrowUpRight size={18} /></span></div></article>)}</div></div></section>
